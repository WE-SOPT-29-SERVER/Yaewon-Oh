...
module.exports = async (req, res) => {
  ...
  if (post.length == 0) {
    return res.status(statusCode.BAD_REQUEST).json(util.fail(statusCode.NOT_FOUND, responseMessage.NO_POST));
  }

  // 삭제 프로세스
  const deleted = { postId };

  res.status(statusCode.OK).json(util.success(statusCode.OK, responseMessage.DELETE_POST_SUCCESS, deleted));
};
